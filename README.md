...
- **意圖路由**：在 `POST /chat/turn` 請求中帶入 `intent: six_chapter_report`，統一規劃代理會調度 `SixChapterReportTool`
- **Prompt Pack**：`prompts/unified_prompts.py` 將規範拆成 `system`、`tool`、`user` 三段提示
- **Schema 驗證**：`models/unified_models.py` 使用 Pydantic 確保六章順序與引用格式
- **測試覆蓋**：
  - 單元測試：`pytest tests/test_six_chapter_unit.py`
  - 網路整合測試：`pytest -m network --host http://127.0.0.1:8000`
...
- 🔧 新增 `SixChapterReportTool`，支援 `six_chapter_report` 意圖
- 🧩 提供 Prompt Pack 與 Pydantic Schema，確保六章順序與引用格式
- 🧪 加入離線單元測試與網路整合測試：`tests/test_six_chapter_unit.py`、`tests/test_chat_turn_integration.py`
